...
# As a rule of thumb, each single iteration takes approx 250ns to complete on a single core of a modern CPU
    ...
    numTiter::Int64 = nump1*numt1*nump2*numt2*1;    # number of T matrix iterations i.e. random p1 p2 points
    ...
    numTiterPerThread::Int64 = nump1*numt1*nump2*numt2*1;    # number of T matrix iterations i.e. random p1 p2 points. Should be > nump1*numt1*nump2*numt2 to ensure good sampling.
...
# Now run the evaluation functions.
    ...
    # ===== DO NOT EDIT THIS SECTION ===== #
        ...
        userInputSerial = (Parameters,numTiter,numSiter,fileLocation,fileName)
        userInputMultiThread = (Parameters,numTiterPerThread,numSiterPerThread,nThreads,fileLocation,fileName)
